...
- ✅ **API rate-limiting (app layer) — implemented 2026-09-22, DEPLOYED + verified live 2026-09-24** (commit `740a8b0`): `express-rate-limit` v8 in the Node API. Global 600/min per-IP cap on `/api` (before JWT verification, so invalid-token floods are throttled too) + tight 20/hr cap on the M2M firmware-upload POST. `app.set('trust proxy', 1)` so `req.ip` is the real client behind Caddy (bump `TRUST_PROXY_HOPS=2` in `.env` at Cloudflare cutover). Env-tunable (`RATE_LIMIT_MAX`, etc.). In-memory store (fine for single instance). Verified live on azul-server: unauth `/api/devices` → `401` with `RateLimit-Policy: 600;w=60` headers. `/health`, `/firmware` downloads, `/uploads` intentionally unthrottled.
- ◐ **WAF / edge rate-limiting — Cloudflare proxy** (decided 2026-09-22). **Config PRE-STAGED 2026-09-24** (decision: author now so cutover is a flip, not a build — edge enforcement is inherently inert until `api.` is proxied + the origin is reachable, so no exposure now): (1) rate-limiting rule `azul-api-baseline` scoped to `http.host eq "api.azul-devices.com"`, ~1000 req/min/IP → block (coarse volumetric backstop above the app-layer 600/min); (2) Cloudflare Free Managed WAF ruleset enabled; (3) SSL/TLS mode → **Full (strict)** (origin already presents a valid public LE cert, so NO Origin CA cert needed). **Executed at forward step:** flip `api.` `proxied:true` (`ddns/cf-ddns.sh` hard-codes `proxied:false` — edit there or toggle in dashboard), open router 443 forward, lock UFW origin-443 to Cloudflare IP ranges, set `TRUST_PROXY_HOPS=2` in server `.env`. Resolves the standing "proxy `api.` through Cloudflare?" open question below. The app-layer limiter above is the defense-in-depth backstop that survives an origin bypass.
